// [pseudo-code] config.ts
// 콘텐츠 스크립트와 서비스워커가 공유하는 상수.
// 모든 주소는 로컬 모의 서버를 가리킨다 — 실제 연동 대상은 이 저장소에 없다.
//
// 빌드: content_scripts의 첫 항목으로 동기 로드된다. import 없이 작성하고 IIFE로
// 선번들한다. 타입은 globals.d.ts의 declare global로 공유한다.
...
// 전역에 등록한다. 서비스워커에는 window가 없으므로 globalThis를 쓴다.
