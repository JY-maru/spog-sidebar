...
//   b) 오리진 — 누가 보냈는지 (self / 임베드 샌드박스 / 신뢰 목록)
...
function isAllowedOrigin(origin: string, kind: 'self' | 'embed-sandbox' | 'trusted-list'): boolean {
  if (kind === 'self') return origin === window.location.origin;
  if (kind === 'embed-sandbox') return origin.endsWith(EMBED_SANDBOX_SUFFIX);
  return TRUSTED_ORIGINS.includes(origin); // manifest의 matches와 대응되는 목록
}

// ── 타입드 레지스트리 ──────────────────────────────────────────
// 새 이벤트를 늘릴 때 손대는 곳은 등록 한 줄뿐이다.
// 같은 타입을 두 번 등록하면 로드 시점에 즉시 에러 — 런타임까지 끌고 가지 않는다.
const handlers = new Map<string, { schema: Schema; handler: Handler }>();

function register(type: string, schema: Schema, handler: Handler) {
  if (handlers.has(type)) throw new Error(`중복 등록된 타입: ${type}`);
  handlers.set(type, { schema, handler });
}
...
// 요청-ID 없이 "kind로 매칭 + 타임아웃이면 null" — 동시 1건 대기를 전제로 한 단순화.
function requestFromEmbedFrame(kind: string, timeoutMs: number): Promise<any | null> {
  return new Promise((resolve) => {
    const timer = setTimeout(() => { cleanup(); resolve(null); }, timeoutMs);
    function onMessage(e: MessageEvent) {
      if (!isAllowedOrigin(e.origin, 'embed-sandbox')) return;
      if (e.data?.type === 'RESPONSE_DATA' && e.data.kind === kind) {
        clearTimeout(timer); cleanup(); resolve(e.data.payload);
      }
    }
    function cleanup() { window.removeEventListener('message', onMessage); }
    window.addEventListener('message', onMessage);
    embedFrame().postMessage({ type: 'REQUEST_DATA', kind }, '*');
  });
}

export function init() {
  register('INTERCEPTED_CASE', CaseSchema, handleInterceptedCase);
  register('INBOUND_CALLBACK_COUNT_UPDATE', CountSchema, (m) => UiController.onInboundCountUpdated(m.count));
  register('CLIENT_CONFIG_UPDATED', ConfigSchema, applyClientConfig);
  // …이하 동일한 모양으로 추가
}

declare const EMBED_SANDBOX_SUFFIX: string;
